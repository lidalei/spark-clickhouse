...
CMD ["/bin/bash", "-c", "python3 store_items_metadata.py --clickhouse-server=clickhouse:9000 --clickhouse-password=${CLICKHOUSE_PASSWORD} --items-dir=${ITEMS_DIR} --metadata-dir=${METADATA_DIR} --num-partitions=${NUM_PARTITIONS} --batch-size=${BATCH_SIZE}"]