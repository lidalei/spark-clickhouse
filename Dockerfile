...
CMD ["/bin/bash", "-c", "python3 store_items_metadata.py --clickhouse-server=${CLICKHOUSE_SERVER} --clickhouse-db=${CLICKHOUSE_DB} --clickhouse-user=${CLICKHOUSE_USER} --clickhouse-password=${CLICKHOUSE_PASSWORD} --items-dir=${ITEMS_DIR} --metadata-dir=${METADATA_DIR}"]